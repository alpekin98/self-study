...
describe('Kulak Project Mainpage Test', () => {
  ...
  it('List length after add', async () => {
    ...
    expect(await homepage.getStationCount()).toBe(1);
  ...
  it('Checks station quantity', async () => {
    homepage.getUrl(localUrl);
    let length = 10;
    let i = 0;
    for (i = 0; i < length; i++) {
    ...
    expect(await homepage.getStationCount()).toBe(length);
    for (i = 0; i < length; i++) {
      homepage.clickDeleteButton(0);
      expect(await homepage.getStationCount()).toBe(length - i - 1);
    }
  });

  it('Checks if form button enabled', async () => {
    homepage.getUrl(localUrl);
    homepage.setName('İstanbul Base Station 42');
    homepage.setLocation('Ankara');
    homepage.setStatus();
    expect(await homepage.getAddButton().isEnabled()).toBe(true);
  });

  it('Checks if form button disabled', async () => {
    homepage.getUrl(localUrl);
    expect(await homepage.getAddButton().isEnabled()).toBe(false);
  });
});
